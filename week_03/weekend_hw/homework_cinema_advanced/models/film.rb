...
class Film
  ...
  def customers()
    sql = "SELECT customers.* FROM customers INNER JOIN tickets ON customer.id = ticket.customer_id INNER JOIN screening ON ticket.screening_id = screening.id WHERE screenings.film_id = #{@id}"
